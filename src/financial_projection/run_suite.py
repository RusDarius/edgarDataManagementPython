...
def print_growth_top_n_console(
    summaries: Sequence[dict[str, Any]],
    *,
    top_n: int = 25,
    growth_lane: str = "own",
) -> list[dict[str, Any]]:
    """Print top-N by own/universe/peer base-scenario implied revenue CAGR."""
    base_rows = [
        row
        for row in summaries
        if row.get("scenario") == "base"
        and str(row.get("growth_lane") or "") == growth_lane
        and bool(row.get("valid"))
    ]
    base_rows.sort(
        key=lambda row: _safe_float(row.get("revenue_cagr_implied_pct"))
        or float("-inf"),
        reverse=True,
    )
    selected = base_rows[: max(0, int(top_n))]
    ...
    print("=" * 118)
    print(
        f"Growth projection top {len(selected)} "
        f"(lane={growth_lane}, base scenario, by implied 5y revenue CAGR)"
    )
    print("=" * 118)
    header = (
        f"{'Rank':<5} {'Symbol':<18} {'RevCAGR':>8} {'EbitdaC':>8} "
        f"{'Y1g':>7} {'Src':<22} {'Name'}"
    )
    print(header)
    print("-" * len(header))
    for rank, row in enumerate(selected, start=1):
        ...
        print(
            f"{rank:<5} {str(row.get('symbol') or ''):<18} "
            f"{_format_pct(row.get('revenue_cagr_implied_pct')):>8} "
            f"{_format_pct(row.get('ebitda_cagr_implied_pct')):>8} "
            f"{_format_pct(row.get('y1_revenue_growth_pct')):>7} "
            f"{str(row.get('growth_lane_source') or '')[:22]:<22} "
            f"{name}"
        )
    print("=" * 118)
    print(
        "Lanes: universe=absolute priors | own=forecast/YoY/CAGR | "
        "peer=blend toward peer growth + margin fade"
    )
    print()
    return selected


def run_growth_projection_suite(
    *,
    all_fields_db: str | Path | None = None,
    scenario_config_path: str | Path | None = None,
    output_root: str | Path | None = None,
    ...
    """
    Growth-first suite: revenue / EBIT / EBITDA / margins on 3 parallel lanes.

    Lanes (not merged): ``universe``, ``own``, ``peer``. Each × bear/base/bull.
    """
    return _run_growth_suite_impl(
        all_fields_db=all_fields_db,
        scenario_config_path=scenario_config_path,
        output_root=output_root,
        min_market_cap_usd=min_market_cap_usd,
...
def run_growth_projection_suite_custom_peer_group(
    *,
    peer_group_symbols: Sequence[str],
    all_fields_db: str | Path | None = None,
    scenario_config_path: str | Path | None = None,
    ...
    """Growth suite with an explicit peer universe (custom ticker group)."""
    cleaned = [
        str(symbol).strip()
        for symbol in peer_group_symbols
        if str(symbol).strip()
...
def _run_growth_suite_impl(
    *,
    ...
    priors = estimate_universe_growth_priors(peer_context)
    summaries, year_grids = project_growth_universe(
        project_rows,
        config=config,
        peer_context_by_symbol=peer_context,
        universe_growth_fraction=float(priors["universe_growth_fraction"] or 0.08),
        universe_ebitda_margin=priors["universe_ebitda_margin"],
        universe_operating_margin=priors["universe_operating_margin"],
        universe_net_margin=priors["universe_net_margin"],
    )

    created_at = datetime.now(tz=timezone.utc)
    persist_result = persist_growth_run(
        output_root=Path(output_root) if output_root is not None else DEFAULT_OUTPUT_ROOT,
        summaries=summaries,
        year_grids=year_grids,
        run_metadata={
            "suite_name": "financial_growth_projection_v1",
            "config_id": config.config_id,
            "config_path": str(config.source_path),
            "horizon_years": config.horizon_years,
            "min_market_cap_usd": resolved_min_mcap,
            ...
            "custom_group_label": custom_group_label,
            "custom_peer_symbols": list(custom_peer_symbols or []),
            "universe_growth_priors": priors,
            "valid_base_own_count": sum(
                1
                for row in summaries
                if row.get("scenario") == "base"
                and row.get("growth_lane") == "own"
                and bool(row.get("valid"))
            ),
        },
        export_parquet=export_parquet,
    ...
    if print_console:
        ...
        print(
            f"Growth lanes projected {len(summaries)} summary rows "
            f"({len(year_grids)} year-grid rows) -> {persist_result['database_path']}"
        )
        print(
            "Universe priors: "
            f"growth={_format_pct((priors['universe_growth_fraction'] or 0) * 100)} "
            f"ebitda_m={_format_pct((priors['universe_ebitda_margin'] or 0) * 100) if priors['universe_ebitda_margin'] is not None else 'n/a'} "
            f"op_m={_format_pct((priors['universe_operating_margin'] or 0) * 100) if priors['universe_operating_margin'] is not None else 'n/a'}"
        )
        top_rows = print_growth_top_n_console(summaries, top_n=top_n, growth_lane="own")
