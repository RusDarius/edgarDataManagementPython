...
## June 2026 Profile Consolidation — `active_manager_v3` (12 lenses)
...
| Sustained momentum + safety | `sustained_momentum_safety_v1` | Peer-relative continuation with extension-aware margin of safety |
...
| Quality continuation | `quality_continuation_v1` | Tape + ROIC/Piotroski floor |
...
| Structural recovery | `value_recovery_v3` | Extension rejection + cheapness mandatory |
...
**Consensus weights (v3.1, sum = 1.00):** breakout 0.14, sustained_momentum_safety 0.10, inflection 0.10, forward_edge_v2 0.09, quality_continuation 0.08, QVC 0.10, DVC 0.07, asymmetric 0.08, value_recovery_v3 0.08, fortress_v2 0.07, fragility 0.06, exhaustion 0.03.

**New derived signals (sustained_momentum_safety_v1):** `momentum_horizon_alignment`, `industry_perf_leadership_1m`, `industry_perf_leadership_3m`, `move_sustainability_score` — computed in `trading_view_move_prediction_analysis.py` from industry perf medians and sustain composites.

**Suites:** `active_manager_v3.json` (production), `baseline_v2.json` (11-profile migration mirror). Frozen `baseline.json` (10 profiles) unchanged for historical DuckDB comparison.

**Overlap tooling:** `scripts/run_move_prediction_profile_overlap_report.py` — critical pairs gated at Jaccard ≤ 0.35 on weeks/months for breakout vs sustained_momentum_safety, breakout vs quality_continuation, asymmetric vs value_recovery_v3, forward_edge_v2 vs QVC.
