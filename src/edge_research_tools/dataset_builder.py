...
def run_symbol_day_feature_snapshot(
    *,
    ...
    output_database_path = context.output_dir / "symbol_day_feature_snapshot.duckdb"
    day_counts_csv = context.output_dir / "symbol_day_snapshot_day_counts.csv"
    report_md = context.output_dir / "symbol_day_feature_snapshot_report.md"
    manifest_path = context.output_dir / "symbol_day_feature_snapshot_manifest.json"
    ...
    try:
        ...
        dynamic_stage_fields = [
            spec
            for spec in selected_fields
            if spec.field_name not in stage_base_aliases
        ]
        ...
        for index, database_path in enumerate(resolved_databases):
            ...
            print(
                f"  [{index + 1}/{n_total}] {source_day_label} ...",
                end="",
                flush=True,
            )
            alias = f"src_{index}"
