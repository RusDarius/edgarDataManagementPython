...
AF_CANDIDATES = {
    ...
    "m1": ("Perf.1M",),
    "m3": ("Perf.3M",),
    "relvol": ("relative_volume_10d_calc",),
    "rsi": ("RSI",),
    "sma20": ("SMA20",),
    "sma50": ("SMA50",),
    "sma200": ("SMA200",),
    ...
    "evebitda": ("enterprise_value_ebitda_ttm", "enterprise_value_ebitda_current"),
    "pb": ("price_book_ratio", "price_book_current", "price_annual_book"),
    "evfcf": ("enterprise_value_to_free_cash_flow_ttm",),
    "opm": ("operating_margin",),
    "adx": ("ADX",),
    "evrev": ("enterprise_value_to_revenue_ttm", "enterprise_value_to_revenue"),
    "atr": ("Average True Range", "ATR"),
    "atrp": ("ATRP",),
    "beta": ("beta_1_year", "Beta.1Y", "beta_1_year_unadjusted"),
    "vol_d": ("Volatility.D",),
}
...
def extract_all_fields(
    ...
    for row in rows:
        ...
        rec = {
            ...
            "m1": _f(row.get("m1")),
            "m3": _f(row.get("m3")),
            "relvol": _f(row.get("relvol")),
            "rsi": _f(row.get("rsi")),
            "adx": _f(row.get("adx")),
            "pe": _f(row.get("pe")),
            "pe_fwd": _f(row.get("pe_fwd")),
            "peg": _f(row.get("peg")),
            "evebitda": _f(row.get("evebitda")),
            "pb": _f(row.get("pb")),
            "evfcf": _f(row.get("evfcf")),
            "opm": _f(row.get("opm")),
            "evrev": _f(row.get("evrev")),
            "pt": _f(row.get("pt")),
            "sma20": _f(row.get("sma20")),
            "sma50": _f(row.get("sma50")),
            "sma200": _f(row.get("sma200")),
            "atr": _f(row.get("atr")),
            "atrp": _f(row.get("atrp")),
            "beta": _f(row.get("beta")),
            "vol_d": _f(row.get("vol_d")),
        }
